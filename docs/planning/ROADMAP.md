...
- **【首要】交互模型：假设透明化 + 单轮澄清**——这是 v0.1 差异化的主线，优先于其余各段。
  - 假设透明化：输出显式区分「用户真实输入」与「系统补足的假设」，不把臆测当事实。
  - 单轮澄清：信息不足时先反问关键问题，用户补充后再生成。严格单轮、有界、无跨会话记忆。
  - 目标：让系统从「替用户写完整报告」变成「帮用户看清想法还差什么」，从而拉开与通用大模型的差距。
