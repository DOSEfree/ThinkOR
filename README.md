...
| 模拟内容归档 | Fake | Real | 必须在页面单独确认后才可保存，因为会把模拟内容写入真实飞书 |

面板保存后的模式对后续请求立即生效。显式设置的系统环境变量优先于 `.env`；页面会提示此类覆盖，避免用户误以为保存未生效。系统不会自动调用真实 LLM 进行测试，“飞书 CLI 引导”中描述的“已授权”也只代表 CLI 准备就绪，首次成功归档才是最终验证。完整配置、飞书身份边界与排障方式见 [SETUP.md](SETUP.md)。

飞书首次使用时，页面会依次提示安装 CLI、在本机终端配置 CLI 应用、完成用户授权和重新检测。

`.env.lock` 是 ThinkOR 在原子更新 `.env` 时保留的跨进程锁文件，不包含 Secret，已被 Git 忽略。服务运行期间请不要手动删除它。
