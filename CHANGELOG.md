...
- 新增极简可用界面（Swiss Style）设计方案，承接 `/api/v1/idea-analysis` 两档响应
- 新增基于 FastAPI + Jinja2 + 原生 fetch 的极简可用界面实现（`/app`）
...
- 新增 `presentation` 层与 `/static` 资源挂载，用单页界面承接澄清档与分析档
- 根据首轮试用反馈精简页头信息、将 9 个分析模块改为纵向排布，并保留二次分析前的澄清记录
- 微调 `/app` 标题、信息块与追问区的版式细节，提升阅读连贯性
