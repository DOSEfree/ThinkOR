...
2. 顶层字段必须包含：
   ...
   open_questions
   analysis
3. input_echo 必须忠实复述【原始想法】本身，尽量原样保留，不要扩写，不要把澄清回答揉进去。
4. assumptions 只写用户没说、但你为了推进分析而补入的前提，不得把假设当事实陈述。
5. 如果进入澄清模式：
...
8. similar_projects 只能根据你已有知识给出，不能假装联网搜索。
9. 宁可显式承认未知，也不要编造确定性；不要为了把报告写满而脑补用户没说过的细节。
""".strip()
    ...
    def build_user_prompt(
        ...
        return (
            ...
            "4. input_echo 只忠实复述【原始想法】本身。\n"
            "5. assumptions 只写用户没说、但你补入的前提。\n"
        )
